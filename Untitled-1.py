...
conn = mysql.connector.connect(host="localhost",port=3306,user="root",password="",database="menumasakann")
...
class Konsumen(Akun) :
    ...
    def tambahPesanan(self):
        ...
        jumlah = jumlah[2:]
        Level = Level[2:]
        query = "insert into pemesanan values('','{}','{}','{}','{}','{}','{}',{},'{}')".format(username, AlamatPengiriman, Nama_masakan, Level, JumlahPemesanan, jumlah, totalHarga, waktuPesan)
        cursor.execute(query)
        conn.commit()

    def tampillkanRiwayatPemesanan(self):
        username = input('masukkan username anda :')
        query="SELECT * FROM pemesanan where username='{}' ".format(username)
        cursor.execute(query)
        data_masakan=cursor.fetchall()
        for data in data_masakan:
            print(data)
...
def jalankanProgram():
    ...
    if menu == 1:
        ...
        if penjualInput == 1:
            ...
            Penjual('Udang rebus').updateMasakan()
            Tampilan.bersih()
